...
def read_image(image_path: Text) -> Image.Image:
  ...
  ret= ImageOps.exif_transpose(Image.open(image_path))
  return ret
...
def camera_matrices_from_annotation(annotation):
  ...
  # width: torch.Tensor, height: torch.Tensor, fx: torch.Tensor, fy: torch.Tensor, cx: torch.Tensor, cy: torch.Tensor=w, h, focal_length_pixels, focal_length_pixels, w // 2, h // 2
  width, height,fx, fy, cx, cy = w, h, focal_length_pixels, focal_length_pixels, w // 2, h // 2
  K = np.array([[fx, 0, cx],
                [0, fy, cy],
                [0, 0, 1]])
  return object_to_world, K
...
def load_scene_data(
        ...
        folder__images_after_exif_transpose=None,
):
  """
  ...
  for i_anno, anno in enumerate(annotations):
    ...
    if folder__images_after_exif_transpose:
      image:Image.Image=read_image(image_path)
      image.save(folder__images_after_exif_transpose/filename)
