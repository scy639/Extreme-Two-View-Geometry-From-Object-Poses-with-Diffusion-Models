...
def let_me_look_at_2d(image_center, K):#look at a new center result in a rotation (camera-img->camera-img_new) and focal length change.
    f_raw = (K[0, 0] + K[1, 1]) / 2
    image_center = image_center - K[:2, 2]
    f_new = np.sqrt(np.linalg.norm(image_center, 2, 0) ** 2 + f_raw ** 2)
    image_center_ = image_center / f_raw
    R_new = look_at_rotation(image_center_)#image_center_: point in normalized image coordinate not in pixels
    return R_new, f_new#  f_new in pixel
