...
class GenMOPDatabase(BaseDatabase):
    def __init__(self, database_name):
        ...
        cameras, images, points3d = read_model(
            f'{GenMOP_ROOT}/{seq_name}/colmap-all/colmap_default-colmap_default/sparse/0')
...
class ShapeNetRenderDatabase(BaseDatabase):
    def __init__(self, database_name):
        ...
        cache_verts_fn = Path(
            f'data/shapenet/shapenet_cache/{self.category}-{self.model_name}-{self.render_setting}-verts.pkl')
    ...
    def parse_model_verts(self):
        ...
        mesh = open3d.io.read_triangle_mesh(
            f'{SHAPENET_ROOT}/{self.category}/{self.model_name}/models/model_normalized.obj')
        return np.asarray(mesh.vertices, np.float32)

    def get_image(self, img_id, ref_mode=False):
        try:
            return imread(
                f'{SHAPENET_RENDER_ROOT}/{self.render_setting}/{self.category}/{self.model_name}/{img_id}.png')[:, :,
                   :3]
        except ValueError:
    ...
    def get_K(self, img_id):
        if img_id in self.img_id2camera:
            ...
            pose, K = read_pickle(
                f'{SHAPENET_RENDER_ROOT}/{self.render_setting}/{self.category}/{self.model_name}/{img_id}-camera.pkl')
    ...
    def get_mask(self, img_id):
        mask = imread(f'{SHAPENET_RENDER_ROOT}/{self.render_setting}/{self.category}/{self.model_name}/{img_id}.png')[:,
               :, 3]
